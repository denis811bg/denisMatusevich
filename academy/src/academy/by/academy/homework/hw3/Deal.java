...
public class Deal {
	...
	public final int MIN_COUNT_ARRAY = 1;
	...
	public void checkBill() {
		...
		if (products == null) {
			...
			for (int i = 0; i < countProduct; i++) {
				checkSum += products[i].getPrice() * products[i].getQuantity() * products[i].discount();
		...
		System.out.println("Кэш покупателя " + buyer.getName() + ": " + buyer.getMoney() + "\n" + "Кэш продавца "
				+ seller.getName() + ": " + seller.getMoney());
		System.out.println("--------------------------------------------");
	}
	...
	public void initMenu() {
		String strMenu = "Добавить товар на склад (введите 1)" + "\nУдалить товар со склада (введите 2)"
	...
	public void initProductArray() {
		...
		while (!str.equals("0")) {
			...
			case "1": {
				System.out.println(
						"Введите наименование вина, стоимость, производителя, количество, тип, крепость и страну производителя: ");
				addProduct(new Wine(scanner.nextLine(), Double.parseDouble(scanner.nextLine().trim()), scanner.nextLine(),
						Integer.parseInt(scanner.nextLine().trim()), scanner.nextLine(),
						Double.parseDouble(scanner.nextLine().trim()), scanner.nextLine()));
			...
			case "2": {
				...
				addProduct(new Cheese(scanner.nextLine(), Double.parseDouble(scanner.nextLine().trim()), scanner.nextLine(),
						Integer.parseInt(scanner.nextLine().trim()), Double.parseDouble(scanner.nextLine().trim())));
				System.out.println("Добавлен товар:\n" + products[countProduct - 1].toString());
				System.out.println("--------------------------------------------");
				System.out.println(strMenu);
				break;
			...
			case "3": {
				...
				addProduct(new Orange(scanner.nextLine(), Double.parseDouble(scanner.nextLine().trim()), scanner.nextLine(),
						Integer.parseInt(scanner.nextLine().trim()), scanner.nextLine()));
				System.out.println("Добавлен товар:\n" + products[countProduct - 1].toString());
				System.out.println("--------------------------------------------");
				System.out.println(strMenu);
